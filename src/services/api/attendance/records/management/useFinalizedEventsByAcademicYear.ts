...
export function useFinalizedEventsByAcademicYear(academicYearId: string) {
     ...
     const fetchData = async () => {
          ...
          try {
               const res = await authFetch(
                    ATTENDANCE_RECORDS_MANAGEMENT_ENPOINTS.GET_FINALIZED_EVENTS_BY_ACADEMIC_YEAR(
                         academicYearId
                    )
               )
